...
def test_add_item_locked():
    inventory = {"items": ["apples", "oranges", "chicken"], "capacity": 10, "locked": True}
...
def test_add_item_capacity():
    inventory = {"items": ["apples", "oranges", "chicken"], "capacity": 3, "locked": False}
    with pytest.raises(ValueError):
...
def test_get_item_count_empty():
    inventory = {"items": [], "capacity": 10, "locked": False}
    result = get_item_count(inventory)
    assert result == 0
